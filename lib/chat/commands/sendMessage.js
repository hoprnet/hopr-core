...
class SendMessage {
    ...
    async complete(line, cb, query) {
        ...
        for (const peerInfo of this.node.peerStore.peers.values()) {
            if ((!query || peerInfo.id.toB58String().startsWith(query)) &&
                !utils_1.isBootstrapNode(this.node, peerInfo.id)) {
        ...
        return cb(undefined, [
            peerInfos.map((peerInfo) => `send ${peerInfo.id.toB58String()}`),
            line,
        ]);
    }
    async selectIntermediateNodes(rl, destination) {
        ...
        let selected = [];
        while (selected.length < constants_1.MAX_HOPS) {
            const lastSelected = selected.length > 0 ? selected[selected.length - 1] : this.node.peerInfo.id;
            const openChannels = await utils_2.getOpenChannelPeerIds(this.node, lastSelected);
